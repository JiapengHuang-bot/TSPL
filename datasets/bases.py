...
class ImageTextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.txt_aug:
            caption_tokens, _ = self.txt_data_aug(caption_tokens.cpu().numpy())
    ...
    def txt_data_aug(self, tokens):
            ...
            if 0 < token < 49405:
                ...
                if prob < 0.15:
                    ...
                    #10% randomly change token to random token
                        ...
                        tokens[i] = random.choice(token_range) # -> rest 10% randomly keep current token
        ...
        if all(l == 0 for l in aug_tokens):
            # at least mask 1
            aug_tokens[1] = tokens[1]
            tokens[1] = mask

        return torch.tensor(tokens), torch.tensor(aug_tokens)
