...
class LinearLayer(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.clf = nn.Sequential(nn.Linear(in_dim, out_dim)).to('cuda')
...
class MMc(nn.Module):
    def __init__(self, args):
        ...
        self.image_classfier = Classifier(0.1, 512, 512, 11003)
        #if self.args.mmc not in ['V']:
            #self.text_encoder = TextEncoder(pretrained_dir=args.pretrained_dir, text_encoder=args.text_encoder)
        self.text_classfier = Classifier(0.1, 512, 512, 11003)
        self.mm_classfier = Classifier(0.0, 1024, 512, 11003)
    ...
    def forward(self, text=None, image=None, label=None, infer=False):
        ...
        output_text = self.text_classfier(text)
        output_image = self.image_classfier(image)

        fusion = torch.cat([text, image], dim=-1)
        ...
        if self.args.mmc in ['SupMMC']:
            mmcLoss = self.mmc_2(text, image, None, None, label)
        ...
        mmcLoss = self.mmc_2(text, image, output_text, output_image, label)
    ...
    def UnSupMMConLoss(self, feature_a, feature_b, temperature=0.07):
        ...
        # compute the mask matrix
        #print(feature_a.shape[0])
        mask = torch.eye(feature_a.shape[0], dtype=torch.float32).to('cuda')

        similarity = torch.matmul(feature_a, feature_b.T)

        # compute logits
        logits = torch.div(similarity, temperature)
        logits_max, _ = torch.max(logits, dim=1, keepdim=True)
        logits = logits - logits_max.detach()
